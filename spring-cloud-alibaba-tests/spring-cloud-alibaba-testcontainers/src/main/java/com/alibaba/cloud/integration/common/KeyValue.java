...
public class KeyValue<K, V> {
		 ...
		 */
		public static <K, V> byte[] encode(K key, Schema<K> keyWriter, V value,
				...
				ByteBuffer byteBuffer = ByteBuffer.allocate(
						4 + keyBytes.length + 4 + valueBytes.length);
				byteBuffer.putInt(key == null ? -1 : keyBytes.length).put(keyBytes)
						.putInt(value == null ? -1 : valueBytes.length).put(valueBytes);
		...
		@Override public boolean equals(Object obj) {
				...
				return Objects.equals(key, another.key) && Objects.equals(value,
						another.value);
		}
		 ...
		 */
		@FunctionalInterface public interface KeyValueDecoder<K, V> {
