...
@Slf4j public class ChaosContainer<SelfT extends ChaosContainer<SelfT>>
		...
		public void putFile(String path, byte[] contents) throws Exception {
				...
				String cmd = String.format("echo %s | base64 -d > %s", base64contents,
						path);
				execCmd("bash", "-c", cmd);
		}
		...
		public CompletableFuture<ContainerExecResult> execCmdAsyncAsUser(String userId,
				...
				DockerClient client = this.getDockerClient();
				String dockerId = this.getContainerId();
				return DockerUtils.runCommandAsyncAsUser(userId, client, dockerId,
						commands);
		}
