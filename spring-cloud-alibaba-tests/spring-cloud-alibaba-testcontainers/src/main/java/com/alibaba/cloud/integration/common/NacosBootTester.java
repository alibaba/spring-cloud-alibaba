...
public abstract class NacosBootTester<ServiceContainerT extends GenericContainer> {
		...
		private final Logger logger = LoggerFactory.getLogger(NacosBootTester.class);
		
		public void vaildateUpdateState(NacosConfig nacosConfig,
				NacosConfigProperties properties, UserProperties userProperties)
				throws NacosException {
				
				ConfigService configService = NacosConfigManager.createConfigService(
						properties);
				String content = uploadFile(userProperties);
				try {
						configService.publishConfig(nacosConfig.getDataId(),
								nacosConfig.getGroup(), content, nacosConfig.getType());
				}
				finally {
