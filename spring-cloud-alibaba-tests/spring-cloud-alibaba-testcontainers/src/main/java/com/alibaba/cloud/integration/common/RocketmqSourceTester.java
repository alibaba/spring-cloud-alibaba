...
public abstract class RocketmqSourceTester<ServiceContainerT extends GenericContainer> {
	...
	public void validateSourceResult(Consumer consumer, int number, String eventType,
			String converterClassName) throws Exception {
	...
	public void validateSourceResultJson(Consumer<KeyValue<byte[], byte[]>> consumer,
			int number, String eventType) throws Exception {
		int recordsNumber = 0;
		// Message<KeyValue<byte[], byte[]>> msg =
		...
		// getSourceType(), consumer.getTopic(), recordsNumber);
	}

	public <K, V> void validateSourceResultAvro(Consumer<KeyValue<K, V>> consumer,
			int number, String eventType) throws Exception {
		int recordsNumber = 0;
		// Message<KeyValue<K,V>> msg = consumer.receive(initialDelayForMsgReceive(),
		// TimeUnit.SECONDS);
		// while(msg != null) {
		// recordsNumber ++;
		...
		// keyRecord.getNativeObject(), valueRecord.getNativeObject());
