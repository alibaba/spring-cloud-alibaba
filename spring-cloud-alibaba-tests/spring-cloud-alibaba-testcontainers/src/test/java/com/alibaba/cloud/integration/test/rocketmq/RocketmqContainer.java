...
@Slf4j
public abstract class RocketmqContainer<SelfT extends RocketmqContainer<SelfT>>
	...
	/**
	 * For debugging purposes, it is useful to have the ability to leave containers
	 * running. This mode can be activated by setting environment variables
	 * CONTAINERS_LEAVE_RUNNING=true and TESTCONTAINERS_REUSE_ENABLE=true After debugging,
	 * one can use this command to kill all containers that were left running: docker kill
	 * $(docker ps -q --filter "label=markcontainer=true")
	 */
	public static final boolean CONTAINERS_LEAVE_RUNNING = Boolean
			.parseBoolean(System.getenv("CONTAINERS_LEAVE_RUNNING"));
	...
	public RocketmqContainer(String clusterName, String hostname, String serviceName,
		...
		this(clusterName, hostname, serviceName, serviceEntryPoint, servicePort, httpPort,
				"/metrics");
	}
	...
	public static void configureLeaveContainerRunning(GenericContainer<?> container) {
		if (CONTAINERS_LEAVE_RUNNING) {
			...
			container.withLabel("markcontainer.random", UUID.randomUUID().toString());
		}
	}
	...
	public void start() {
		...
		super.start();
		afterStart();
		log.info("[{}] Start pulsar service {} at container {}", getContainerName(),
				serviceName, getContainerId());
	}
