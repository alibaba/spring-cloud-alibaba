 ...
 * {@link TimerTask} on time. {@link HashedWheelTimer}, on every tick, will check if there
 * are any {@link TimerTask}s behind the schedule and execute them.
 * <p>
 ...
 * <p>
 * {@link HashedWheelTimer} maintains a data structure called 'wheel'. To put simply, a
 * wheel is a hash table of {@link TimerTask}s whose hash function is 'dead line of the
 * task'. The default number of ticks per wheel (i.e. the size of the wheel) is 512. You
 * could specify a larger value if you are going to schedule a lot of timeouts.
 *
 * <h3>Do not create many instances.</h3>
 * <p>
 * {@link HashedWheelTimer} creates a new thread whenever it is instantiated and started.
 * Therefore, you should make sure to create only one instance and share it across your
 * application. One of the common mistakes, that makes your application unresponsive, is
 * to create a new instance for every connection.
 *
 * <h3>Implementation Details</h3>
 * <p>
 * {@link HashedWheelTimer} is based on
 * <a href="http://cseweb.ucsd.edu/users/varghese/">George Varghese</a> and Tony Lauck's
 * paper, <a href="http://cseweb.ucsd.edu/users/varghese/PAPERS/twheel.ps.Z">'Hashed and
 * Hierarchical Timing Wheels: data structures to efficiently implement a timer
 * facility'</a>. More comprehensive slides are located
...
public class HashedWheelTimer implements Timer {
	 ...
	 * ({@link Executors#defaultThreadFactory()}), default tick duration, and default
	...
	/**
	 * Creates a new timer with the default thread factory
	 * ({@link Executors#defaultThreadFactory()}) and default number of ticks per wheel.
	...
	/**
	 * Creates a new timer with the default tick duration and default number of ticks per
	 * wheel.
	 ...
	 * @param unit the time unit of the {@code tickDuration}
	 * @param ticksPerWheel the size of the wheel
	 * @param maxPendingTimeouts The maximum number of pending timeouts after which call
	 * to {@code newTimeout} will result in
	 * {@link java.util.concurrent.RejectedExecutionException} being thrown. No maximum
	...
	protected void finalize() throws Throwable {
		try {
			...
			// This object is going to be GCed and it is assumed the ship has sailed to do
			// a proper shutdown. If
			// we have not yet shutdown then we want to make sure we decrement the active
			// instance count.
			if (WORKER_STATE_UPDATER.getAndSet(this,
					WORKER_STATE_SHUTDOWN) != WORKER_STATE_SHUTDOWN) {
	 ...
	 * @throws IllegalStateException if this timer has been {@linkplain #stop() stopped}
	 * already
	 */
	public void start() {
	...
	private static void reportTooManyInstances() {
		String resourceType = ClassUtils.getSimplaName(HashedWheelTimer.class);
	...
	private final class Worker implements Runnable {
		...
		public void run() {
			...
			if (startTime == 0) {
				// We use 0 as an indicator for the uninitialized value here, so make sure
				// it's not 0 when initialized.
				startTime = 1;
			}
		...
		private void transferTimeoutsToBuckets() {
			// transfer only max. 100000 timeouts per tick to prevent a thread to stale
			// the workerThread when it just
		...
		/**
		 * calculate goal nanoTime from startTime and current tick number, then wait until
		...
		private long waitForNextTick() {
			...
			for (;;) {
				...
				try {
					...
					if (WORKER_STATE_UPDATER
							.get(HashedWheelTimer.this) == WORKER_STATE_SHUTDOWN) {
	...
	private static final class HashedWheelTimeout implements Timeout {
		 ...
		 * Worker.transferTimeoutsToBuckets() before the HashedWheelTimeout will be added
		...
		public boolean cancel() {
			// only update the state it will be removed from HashedWheelBucket on next
			// tick.
			...
			// So this means that we will have a GC latency of max. 1 tick duration which
			// is good enough. This way
		...
		public String toString() {
			...
			String simpleClassName = ClassUtils.getSimplaName(this.getClass());

			StringBuilder buf = new StringBuilder(192).append(simpleClassName).append('(')
					.append("deadline: ");
			if (remaining > 0) {
	...
	/**
	 * Bucket that stores HashedWheelTimeouts. These are stored in a linked-list like
	 * datastructure to allow easy removal of HashedWheelTimeouts in the middle. Also the
