...
public class NacosEnvironmentPostProcessor implements EnvironmentPostProcessor {
	...
	public void postProcessEnvironment(ConfigurableEnvironment environment, SpringApplication application) {
		Boolean eagerLoad = environment.getProperty(NacosConfigProperties.PREFIX + ".eagerLoad.enabled", Boolean.class, false);
		if (!eagerLoad || environment.getPropertySources().contains(NACOS_PROPERTY_SOURCE_NAME)) {
			return;
		}

		Binder binder = Binder.get(environment);
		NacosConfigProperties nacosConfigProperties = binder.bind(NacosConfigProperties.PREFIX, NacosConfigProperties.class).get();
