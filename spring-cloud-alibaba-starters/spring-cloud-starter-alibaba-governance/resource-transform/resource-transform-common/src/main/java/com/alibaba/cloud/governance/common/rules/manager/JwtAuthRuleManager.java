...
public class JwtAuthRuleManager {
    ...
    private static boolean judgePresenter(String presenter, JwtAuthRule andRules) {
        return andRules == null || andRules.getAuthPresenters().isEmpty() || andRules.getAuthPresenters().getRules().stream().allMatch(orRules -> {
            boolean flag = orRules.getRules().stream().anyMatch(jwtPresenter -> StringMatchUtil.matchStr(presenter, jwtPresenter));
            return orRules.isNot() != flag;
        });
    }

    private static boolean judgePrincipal(String subject, String issuer, JwtAuthRule andRules) {
        return andRules == null || andRules.getAuthPresenters().isEmpty() || andRules.getRequestPrincipals().getRules().stream().allMatch(orRules -> {
            boolean flag = orRules.getRules().stream().anyMatch(jwtPrincipal -> StringMatchUtil.matchStr(issuer + "/" + subject, jwtPrincipal));
            return orRules.isNot() != flag;
        });
    }

    private static boolean judgeAudience(List<String> audiences, JwtAuthRule andRules) {
        return andRules == null || andRules.getAuthPresenters().isEmpty() || andRules.getAuthAudiences().getRules().stream().allMatch(orRules -> {
            boolean flag = audiences.stream().anyMatch(audStr -> orRules.getRules().stream().anyMatch(jwtAudience -> StringMatchUtil.matchStr(audStr, jwtAudience)));
            return orRules.isNot() != flag;
        });
    }

    private static boolean judgeClaims(JwtClaims jwtClaims, JwtAuthRule andRules) {
        return andRules == null || andRules.getAuthPresenters().isEmpty() || andRules.getAuthClaims().entrySet().stream().allMatch(allHeader -> {
            ...
            return allHeader.getValue().getRules().stream().allMatch(andRule -> {
                ...
                boolean flag = andRule.getRules().stream().anyMatch(orRule -> orRule.hasOneOf() && orRule.getOneOf().hasStringMatch() && StringMatchUtil.matchStr(claimValue, orRule.getOneOf().getStringMatch()));
    ...
    private static boolean judgeJwt(Map<String, JwtAuthRule> rules, JwtClaims jwtClaims) throws MalformedClaimException {
        ...
        return rules.values().stream().anyMatch(andRules -> judgePresenter(presenter, andRules) && judgePrincipal(subject, issuer, andRules) && judgeAudience(audiences, andRules) && judgeClaims(jwtClaims, andRules));
    }
}
