...
import com.alibaba.cloud.governance.common.rules.auth.HttpHeaderRule;
...
public class HeaderRuleManager {
    ...
    private static boolean judgeHttpHeaderRule(Map<String, HttpHeaderRule> rules, HttpHeaders headers) {
        return rules.values().stream().anyMatch(andRules -> judgeHttpHeaderRule(andRules, headers));
    }

    private static boolean judgeHttpHeaderRule(HttpHeaderRule andRules, HttpHeaders headers) {
        return andRules == null || andRules.getHeaders().isEmpty() || andRules.getHeaders().entrySet().stream().allMatch(allHeader -> {
            String key = allHeader.getKey();
            return allHeader.getValue().getRules().stream().allMatch(andRule -> {
                boolean flag = andRule.getRules().stream().anyMatch(orRule -> HeaderMatchUtil.matchHeader(headers, key, orRule));
