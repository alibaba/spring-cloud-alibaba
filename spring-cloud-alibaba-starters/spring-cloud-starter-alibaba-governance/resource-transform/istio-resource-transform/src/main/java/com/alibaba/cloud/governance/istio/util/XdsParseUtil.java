    ...
    private static void resolvePrincipal(String name, Principal principal, boolean isAllowed) {
        ...
        for (Principal andId : andIds.getIdsList()) {
            ...
            for (Map.Entry<String, List<ListMatcher>> entry : authClaims.entrySet()) {
                AndRule<ListMatcher> authClaimEntries = authClaimMap.getOrDefault(entry.getKey(), new AndRule<>());
            ...
            for (Map.Entry<String, List<HeaderMatcher>> entry : headers.entrySet()) {
                AndRule<HeaderMatcher> headerEntries = headerMap.getOrDefault(entry.getKey(), new AndRule<>());
        ...
        if (!headerMap.isEmpty()) {
            HeaderRuleManager.addHttpHeaderRule(new HttpHeaderRule(name, headerMap), isAllowed);
        }
    }
                ...
                if (orRule.hasHeader()) {
                    switch (orRule.getHeader().getName()) {
                        case HEADER_NAME_AUTHORITY:
                            hosts.add(HeaderMatchUtil.headerMatch2StringMatch(orRule.getHeader()));
                            break;
                        case HEADER_NAME_METHOD:
                            methods.add(HeaderMatchUtil.headerMatch2StringMatch(orRule.getHeader()));
                            break;
                    }
                }
