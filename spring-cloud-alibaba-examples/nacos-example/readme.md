...
## Nacos Server 3.1.1 is properly configured and started

In Nacos 3.1.1, functions related to user authentication are added. When starting Nacos Server for the first time, it needs to be configured correctly to avoid the problem of startup failure.

### Download Nacos Server

> The Nacos serv version used in this example is 3.1.1!

Nacos supports both direct download and source code construction. **Nacos Server version 3.1.1 is recommended for Spring Cloud Alibaba 2022.x.**
...
Open the `\nacos-server-3.1.1\conf\application.properties` configuration file and modify the following configuration items:

#### Configure the data source

Take the MySQL database as an example here, and use the `nacos-server-3.1.1\conf\mysql-schema.sql` initialization database table file. Modify the following configuration as well
...
**Note: If it is not enabled, login failure exception will occur in 3.1.1!**
...
Authentication is required when using the Open api interface in nacos server 3.1.1: For more details, please refer to: [Nacos api authentication](https://nacos.io/zh-cn/docs/auth.html)
