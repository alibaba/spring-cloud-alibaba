...
${spring.application.group}:${spring.application.name}-${spring.profiles.active}.${file-extension}。 Group 的值目前只支持 DEFAULT_GROUP，因此在使用过程中请额外注意改值配置的正确性，不要和 `spring.application.group` 这个配置搞混了。
