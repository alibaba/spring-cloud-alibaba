...
如果要在您的项目中使用 Nacos 来实现服务发现，使用 group ID 为 `com.alibaba.cloud` 和 artifact ID 为 `alibaba-nacos-discovery-spring-cloud-starter` 的 starter。
...
alibaba-nacos-discovery-spring-cloud-starter 在实现的时候提供了一个EndPoint,EndPoint的访问地址为 `http://ip:port/actuator/nacos-discovery`。
EndPoint 的信息主要提供了两类:
...
更多关于 alibaba-nacos-discovery-spring-cloud-starter 的 starter 配置项如下所示:
