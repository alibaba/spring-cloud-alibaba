...
spring-cloud-starter-alibaba-nacos-discovery 在实现的时候提供了一个EndPoint,EndPoint的访问地址为 `https://ip:port/actuator/nacos-discovery`。
EndPoint 的信息主要提供了两类:
...
启动后，访问 Consumer 提供出来的 `https://ip:port/echo/app-name` 接口。我这里测试启动的 port是 8082。访问结果如下所示：
