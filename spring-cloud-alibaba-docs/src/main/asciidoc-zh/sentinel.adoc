...
如果要在您的项目中引入 Sentinel，使用 group ID 为 `com.alibaba.cloud` 和 artifact ID 为 `alibaba-sentinel-spring-cloud-starter` 的 starter。
...
#spring.cloud.sentinel.datasource.ds1.file.converter-class=com.alibaba.cloud.alibaba.cloud.examples.JsonFlowRuleListConverter
#spring.cloud.sentinel.datasource.ds1.file.rule-type=flow
