...
如果要在您的项目中使用 Nacos 来实现应用的外部化配置，使用 group ID 为 `com.alibaba.cloud` 和 artifact ID 为 `alibaba-nacos-config-spring-cloud-starter` 的 starter。
...
alibaba-nacos-config-spring-cloud-starter 对于 yaml 格式也是完美支持的。这个时候只需要完成以下两步：
...
alibaba-nacos-config-spring-cloud-starter 也支持配置的动态更新，启动 Spring Boot 应用测试的代码如下：
...
alibaba-nacos-config-spring-cloud-starter 在加载配置的时候，不仅仅加载了以 dataid 为 `${spring.application.name}.${file-extension:properties}`   为前缀的基础配置，还加载了dataid为 `${spring.application.name}-${profile}.${file-extension:properties}` 的基础配置。在日常开发中如果遇到多套环境下的不同配置，可以通过Spring 提供的 `${spring.profiles.active}` 这个配置项来配置。
