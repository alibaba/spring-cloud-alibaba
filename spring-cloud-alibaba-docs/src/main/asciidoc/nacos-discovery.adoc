...
|Perfer use hostname to registered|`spring.cloud.nacos.discovery.prefer-hostname`|false|Will using the hostname converted from ip, to register this service. if converted failed, using ip as fallback.
