...
spring-cloud-starter-alibaba-nacos-discovery provides an EndPoint, and the address is `https://ip:port/actuator/nacos-discovery`.
...
public class NacosConsumerApp {
    ...
    public class NacosController{
        ...
        public String echoAppName(){
            ...
            String path = String.format("https://%s:%s/echo/%s",serviceInstance.getHost(),serviceInstance.getPort(),appName);
...
Next, access the `https://ip:port/echo/app-name` interface provided by the consumer. Here we started the port of 8082. The access result is shown below：
