...
Consumer applications may not be as simple as launching a Provider application. Because the Consumer side needs to call the REST service provided by the Provider side.
The client load balancing components used in the Spring Cloud Alibaba project of the 2.2.x branch are:
[xml]
----
<dependency>
     <groupId>org.springframework.cloud</groupId>
...
For external users using the 2.2.x branch, import spring-cloud-starter-alibaba-nacos-discovery to call downstream services normally.
But in the spring-cloud-alibaba-nacos-discovery related branch in the 2021.x branch, because Spring Cloud removed the Ribbon component from the 2020.0.0 version. So we made some adjustments.
Replace the client load balancing component from Ribbon with loadbalancer, as follows:
[xml]
----
<dependency>
     <groupId>org.springframework.cloud</groupId>
...
**It should be noted that this loadbalancer dependency is not introduced by default in the Spring Cloud Alibaba 2021.x+ branch, and users need to add it in the pom.xml file. Otherwise, the downstream service call may fail! **

In the example we use the most primitive way,
That is, explicitly use the combination of LoadBalanceClient and RestTemplate to access.
The configuration of pom.xml and application.properties can refer to 1.2 Summary. The sample code to start a Consumer application is as follows:


NOTE: It is also accessible through RestTemplate and FeignClient with load balancing.
