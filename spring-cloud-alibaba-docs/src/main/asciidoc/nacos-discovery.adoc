...
You do not need to configure the IPv4 address and IPv6 address of the microservice to register the registry by default, the IPv4 address is under the IP field of Nacos, and the IPv6 address is in the metadata field of Nacos. When consumer calls provider, it will select the appropriate IP address based on its own IP address and the IP address of the provider.
If the provider registers a dual-stack address, the provider will choose its own IPv6 address for communication under the condition that it has an IPv6 address. In the case that the provider has only a single stack, the consumer will select the corresponding IP address for the call.
