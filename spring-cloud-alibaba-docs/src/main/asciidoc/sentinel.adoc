...
The Sentinel community has adapted support for WebFlux since version 1.5.0, and if you want to use WebFlux, you need to introduce WebFlux dependencies into your project
[source,xml]
----
<dependency>
...
----
And write the following configuration class
[source,java]
----
@Configuration
...
The Sentinel Reactor adaptation implements the corresponding Sentinel Operator for Mono and Flux respectively, so as to import Sentinel's relevant logic when various events are triggered. At the same time, Sentinel provides SentinelReactorTransformer on the upper layer for loading the corresponding operator during assembly, and users only need to transform through the transform operator when using it:
[source,java]
----
...
For more information, please refer to the provided by the Spring Cloud Alibaba community https://github.com/alibaba/spring-cloud-alibaba/tree/2021.x/spring-cloud-alibaba-examples/sentinel-example/sentinel-webflux-example[sentinel-webflux-example]
