...
Spring Cloud Alibaba aims to provide a one-stop solution for microservices development. This projects includes the required components for developing distributed applications and services, so that developers can develop distributed applications easily with the Spring Cloud programming models.
